    ...
    int src_x, int src_y, int src_w, int src_h)
{
    ...
    if (sel)
    {
        if (coll)
        {
            if (MenuMouseDown)
                ...
                frmMain.renderRect(x - 2, y - 2, 36, 36, 0.8f, 1.0f, 0.8f, 1.0f, true);
        }
        else
    ...
    else if (coll)
    {
        if (MenuMouseDown)
            ...
            frmMain.renderRect(x - 2, y - 2, 36, 36, 1.0f, 1.0f, 1.0f, 1.0f, true);
    }
    // background:
    ...
    frmMain.renderTextureScale(dst_x, dst_y, dst_w, dst_h, im, src_x, src_y, src_w, src_h);
...
{
    // NPC GUI
    frmMain.renderRect(e_ScreenW - 240, 40, 240, ScreenH - 40, 0.7f, 0.7f, 0.9f, 1.0f, true);
...
void EditorScreen::UpdateEventSettingsScreen()
{
        ...
        if (UpdateButton(20 + 4, 100 + 4, GFX.ECursor[1], false, 0, 0, 32, 32))
        {
            if (m_special_subpage == 0)
            {
                for (int s = 0; s <= maxSections; s++)
                    Events[m_current_event].level[s] = level[s];
            }
            else
                Events[m_current_event].level[m_special_subpage-1] = level[m_special_subpage-1];
            m_special_page = SPECIAL_PAGE_EVENT_SETTINGS;
        }
    ...
    frmMain.renderRect(0, ScreenH - 180, e_ScreenW - 240, 180, 0.6f, 0.6f, 0.8f, 1.0f, true);
    if (m_special_subpage > 0 && UpdateButton(40 + 4, ScreenH - 180 + 4, GFX.NPC_modes, false, 0, 32*1, 32, 32))
        m_special_subpage --;
    if (m_special_subpage < maxSections + 1 && UpdateButton(320 + 4, ScreenH - 180 + 4, GFX.NPC_modes, false, 0, 32*2, 32, 32))
        m_special_subpage ++;
    // description of possibilities
    SuperPrint("KEEP", 3, 132, ScreenH - 140);
    SuperPrint("RESET", 3, 226, ScreenH - 140);
    SuperPrint("SET", 3, 340, ScreenH - 140);

    SuperPrint("MUSIC", 3, 4, ScreenH - 110);
    SuperPrint("BG", 3, 4, ScreenH - 70);
    SuperPrint("BOUNDS", 3, 4, ScreenH - 30);
    // subpage - 1 is the internal section ID; subpage 0 is all sections.
    if (m_special_subpage == 0)
    {
        SuperPrint("ALL SECTIONS", 3, 90, ScreenH - 174);
        // music
        bool all_keep = true;
        bool all_reset = true;
        bool all_set = true;
        ...
        for (int s = 0; s <= maxSections; s++)
        {
            if (Events[m_current_event].Music[s] != -1)
                all_keep = false;
            if (Events[m_current_event].Music[s] != -2)
                all_reset = false;
            if (Events[m_current_event].Music[s] < 0)
                all_set = false;
            else if (set_same == -1)
                set_same = Events[m_current_event].Music[s];
            else if (set_same != Events[m_current_event].Music[s])
                all_set = false;
        }
        if (UpdateButton(150 + 4, ScreenH - 120 + 4, GFX.NPC_modes, all_keep, 0, 32*31, 32, 32) && !all_keep)
        {
            for (int s = 0; s <= maxSections; s++)
                Events[m_current_event].Music[s] = -1;
        }
        if (UpdateButton(250 + 4, ScreenH - 120 + 4, GFX.NPC_modes, all_reset, 0, 32*8, 32, 32) && !all_reset)
        {
            for (int s = 0; s <= maxSections; s++)
                Events[m_current_event].Music[s] = -2;
        }
        if (UpdateButton(350 + 4, ScreenH - 120 + 4, GFX.ECursor[1], all_set, 0, 0, 32, 32))
        ...
        for (int s = 0; s <= maxSections; s++)
        {
            if (Events[m_current_event].Background[s] != -1)
                all_keep = false;
            if (Events[m_current_event].Background[s] != -2)
                all_reset = false;
            if (Events[m_current_event].Background[s] < 0)
                all_set = false;
            else if (set_same == -1)
                set_same = Events[m_current_event].Background[s];
            else if (set_same != Events[m_current_event].Background[s])
                all_set = false;
        }
        if (UpdateButton(150 + 4, ScreenH - 80 + 4, GFX.NPC_modes, all_keep, 0, 32*31, 32, 32) && !all_keep)
        {
            for (int s = 0; s <= maxSections; s++)
                Events[m_current_event].Background[s] = -1;
        }
        if (UpdateButton(250 + 4, ScreenH - 80 + 4, GFX.NPC_modes, all_reset, 0, 32*8, 32, 32) && !all_reset)
        {
            for (int s = 0; s <= maxSections; s++)
                Events[m_current_event].Background[s] = -2;
        }
        if (UpdateButton(350 + 4, ScreenH - 80 + 4, GFX.ECursor[1], all_set, 0, 0, 32, 32))
        ...
        for (int s = 0; s <= maxSections; s++)
        {
            if ((int)Events[m_current_event].level[s].X == -1)
            {
                all_reset = false;
                all_set = false;
            }
            else if ((int)Events[m_current_event].level[s].X == -2)
            {
                all_set = false;
                all_keep = false;
            }
        ...
        if (UpdateButton(150 + 4, ScreenH - 40 + 4, GFX.NPC_modes, all_keep, 0, 32*31, 32, 32) && !all_keep)
        {
            for (int s = 0; s <= maxSections; s++)
                Events[m_current_event].level[s].X = -1;
        }
        if (UpdateButton(250 + 4, ScreenH - 40 + 4, GFX.NPC_modes, all_reset, 0, 32*8, 32, 32) && !all_reset)
        {
            for (int s = 0; s <= maxSections; s++)
                Events[m_current_event].level[s].X = -2;
        }
        if (UpdateButton(350 + 4, ScreenH - 40 + 4, GFX.ECursor[1], all_set, 0, 0, 32, 32))
            m_special_page = SPECIAL_PAGE_EVENT_BOUNDS;
    }
    else
    {
        SuperPrint("SECTION "+std::to_string(m_special_subpage), 3, 120, ScreenH - 174);
        if (UpdateButton(150 + 4, ScreenH - 120 + 4, GFX.NPC_modes, Events[m_current_event].Music[m_special_subpage-1] == -1, 0, 32*31, 32, 32))
        {
            Events[m_current_event].Music[m_special_subpage-1] = -1;
        }
        if (UpdateButton(250 + 4, ScreenH - 120 + 4, GFX.NPC_modes, Events[m_current_event].Music[m_special_subpage-1] == -2, 0, 32*8, 32, 32))
        {
            Events[m_current_event].Music[m_special_subpage-1] = -2;
        }
        if (UpdateButton(350 + 4, ScreenH - 120 + 4, GFX.ECursor[1], Events[m_current_event].Music[m_special_subpage-1] >= 0, 0, 0, 32, 32))
            m_special_page = SPECIAL_PAGE_EVENT_MUSIC;
        if (UpdateButton(150 + 4, ScreenH - 80 + 4, GFX.NPC_modes, Events[m_current_event].Background[m_special_subpage-1] == -1, 0, 32*31, 32, 32))
        {
            Events[m_current_event].Background[m_special_subpage-1] = -1;
        }
        if (UpdateButton(250 + 4, ScreenH - 80 + 4, GFX.NPC_modes, Events[m_current_event].Background[m_special_subpage-1] == -2, 0, 32*8, 32, 32))
        {
            Events[m_current_event].Background[m_special_subpage-1] = -2;
        }
        if (UpdateButton(350 + 4, ScreenH - 80 + 4, GFX.ECursor[1], Events[m_current_event].Background[m_special_subpage-1] >= 0, 0, 0, 32, 32))
            m_special_page = SPECIAL_PAGE_EVENT_BACKGROUND;
        if (UpdateButton(150 + 4, ScreenH - 40 + 4, GFX.NPC_modes, (int)Events[m_current_event].level[m_special_subpage-1].X == -1, 0, 32*31, 32, 32))
        {
            Events[m_current_event].level[m_special_subpage-1].X = -1;
        }
        if (UpdateButton(250 + 4, ScreenH - 40 + 4, GFX.NPC_modes, (int)Events[m_current_event].level[m_special_subpage-1].X == -2, 0, 32*8, 32, 32))
        {
            Events[m_current_event].level[m_special_subpage-1].X = -2;
        }
        if (UpdateButton(350 + 4, ScreenH - 40 + 4, GFX.ECursor[1], (int)Events[m_current_event].level[m_special_subpage-1].X != -1 && (int)Events[m_current_event].level[m_special_subpage-1].X != -2, 0, 0, 32, 32))
            m_special_page = SPECIAL_PAGE_EVENT_BOUNDS;
    }
...
void UpdateStartLevelEventBounds()
{
    ...
    Events[0].level[0] = level[0];
    // not sure why 800 is also used for height in the default code, but I will stick with it.
    if (Events[0].AutoX < 0)
        Events[0].level[0].X = Events[0].level[0].Width - 800;
    else if (Events[0].AutoX > 0)
        Events[0].level[0].Width = Events[0].level[0].X + 800;
    if (Events[0].AutoY < 0)
        Events[0].level[0].Y = Events[0].level[0].Height - 800;
    else if (Events[0].AutoY > 0)
        Events[0].level[0].Height = Events[0].level[0].Y + 800;
}
...
void EditorScreen::UpdateSelectListScreen()
{
    ...
    else if (m_special_page == SPECIAL_PAGE_EVENT_BACKGROUND)
    {
        if (m_special_subpage > 0)
        {
            ...
            target = &Events[m_current_event].Background[m_special_subpage-1];
        }
        else
        {
    ...
    else if (m_special_page == SPECIAL_PAGE_EVENT_MUSIC)
    {
        if (m_special_subpage > 0)
        {
            ...
            target = &Events[m_current_event].Music[m_special_subpage-1];
        }
        else
        {
    ...
    for (int i = 0; i < 20; i++)
    {
        ...
        if (index != -1 || m_special_page == SPECIAL_PAGE_LEVEL_EXIT)
        {
            ...
            else if (m_special_page == SPECIAL_PAGE_EVENT_BACKGROUND)
            {
                ...
                for (int s = 0; s <= maxSections; s++)
                {
                    if (Events[m_current_event].Background[s] != index)
                        sel = false;
                }
            }
            else if (m_special_page == SPECIAL_PAGE_EVENT_MUSIC)
            {
                ...
                for (int s = 0; s <= maxSections; s++)
                {
                    if (Events[m_current_event].Music[s] != index)
                        sel = false;
                }
            }
            ...
            if (UpdateButton(x + 4, y + 4, GFX.ECursor[2], sel, 0, 0, 32, 32) && !sel)
            {
                ...
                {
                    for (int s = 0; s <= maxSections; s++)
                        Events[m_current_event].Background[s] = index;
                }
                else if (m_special_page == SPECIAL_PAGE_EVENT_MUSIC)
                {
                    for (int s = 0; s <= maxSections; s++)
                        Events[m_current_event].Music[s] = index;
                }
...
void EditorScreen::UpdateEventsSubScreen()
{
    ...
    else if (EditorCursor.Mode == OptCursor_t::LVL_BLOCKS)
    {
        ...
        return;
    int page_max = numEvents / 10;
    SuperPrint("PAGE " + std::to_string(m_events_page+1) + " OF " + std::to_string(page_max+1), 3, e_ScreenW - 228, ScreenH - 60);
    if (m_events_page > 0 && UpdateButton(e_ScreenW - 160 + 4, ScreenH - 40 + 4, GFX.NPC_modes, false, 0, 32*1, 32, 32))
        m_events_page --;
    if (m_events_page < page_max && UpdateButton(e_ScreenW - 120 + 4, ScreenH - 40 + 4, GFX.NPC_modes, false, 0, 32*2, 32, 32))
        m_events_page ++;
...
void EditorScreen::UpdateLayersScreen()
{
    ...
    {
        SuperPrint("PAGE " + std::to_string(m_layers_page+1) + " OF " + std::to_string(page_max+1), 3, e_ScreenW - 228, ScreenH - 60);
        if (m_layers_page > 0 && UpdateButton(e_ScreenW - 160 + 4, ScreenH - 40 + 4, GFX.NPC_modes, false, 0, 32*1, 32, 32))
            m_layers_page --;
        if (m_layers_page < page_max && UpdateButton(e_ScreenW - 120 + 4, ScreenH - 40 + 4, GFX.NPC_modes, false, 0, 32*2, 32, 32))
            m_layers_page ++;
    }
...
{
    // Block GUI
    frmMain.renderRect(e_ScreenW - 120, 40, 240, ScreenH - 40, 0.7f, 0.7f, 0.9f, 1.0f, true);
    // Page selector
...
void EditorScreen::UpdateLevelScreen()
{
    ...
    frmMain.renderRect(0, ScreenH - 240, e_ScreenW - 240, 240, 0.6f, 0.6f, 0.8f, 1.0f, true);

    // level name - LevelName
    SuperPrint("LEVEL NAME:", 3, 10 + 44, ScreenH - 240 + 2);
    SuperPrint(EditorCursor.WorldLevel.LevelName.substr(0, 19), 3, 10 + 44, ScreenH - 240 + 20);
    if (EditorCursor.WorldLevel.LevelName.length() > 19)
        SuperPrint(EditorCursor.WorldLevel.LevelName.substr(19), 3, 10 + 44 + 18, ScreenH - 240 + 38);
    if (UpdateButton(10 + 4, ScreenH - 240 + 4, GFX.NPC_modes, false, 0, 32*17, 32, 32))
        EditorCursor.WorldLevel.LevelName = GetTextInput("Level name", EditorCursor.WorldLevel.LevelName);

    // level filename - FileName
    SuperPrint("FILENAME:", 3, 10 + 44, ScreenH - 180 + 2);
    if (!EditorCursor.WorldLevel.FileName.empty())
        SuperPrint(EditorCursor.WorldLevel.FileName, 3, 10 + 44, ScreenH - 180 + 20);
    else
    ...
    if (!EditorCursor.WorldLevel.FileName.empty())
    {
        ...
        if (EditorCursor.WorldLevel.StartWarp > 0 && UpdateButton(50 + 4, ScreenH - 180 + 40 + 4, GFX.NPC_modes, false, 0, 32*1, 32, 32))
            EditorCursor.WorldLevel.StartWarp -= 1;
        if (EditorCursor.WorldLevel.StartWarp < maxWarps && UpdateButton(280 + 4, ScreenH - 180 + 40 + 4, GFX.NPC_modes, false, 0, 32*2, 32, 32))
            EditorCursor.WorldLevel.StartWarp += 1;
        SuperPrint("ENTRANCE:", 3, 50 + 44, ScreenH - 180 + 40 + 2);
        if (EditorCursor.WorldLevel.StartWarp != 0)
            SuperPrint("WARP " + std::to_string(EditorCursor.WorldLevel.StartWarp), 3, 50 + 44, ScreenH - 180 + 40 + 20);
        else
            SuperPrint("NORMAL", 3, 50 + 44, ScreenH - 180 + 40 + 20);

        // exits
        SuperPrint("PATH UNLOCKS", 3, 10 + 44, ScreenH - 180 + 80 + 2);

        if (UpdateButton(10 + 4, ScreenH - 180 + 100 + 4, GFX.ECursor[1], false, 0, 0, 32, 32))
        {
            m_special_page = SPECIAL_PAGE_LEVEL_EXIT;
            m_special_subpage = 1;
        }
        SuperPrint("UP", 3, 10 + 44, ScreenH - 180 + 102);
        SuperPrint(list_level_exit_names[EditorCursor.WorldLevel.LevelExit[1]+1], 3, 10 + 44, ScreenH - 180 + 120);

        if (UpdateButton((e_ScreenW - 240)/2 + 10 + 4, ScreenH - 180 + 100 + 4, GFX.ECursor[1], false, 0, 0, 32, 32))
        {
            m_special_page = SPECIAL_PAGE_LEVEL_EXIT;
            m_special_subpage = 3;
        }
        SuperPrint("DOWN", 3, (e_ScreenW - 240)/2 + 10 + 44, ScreenH - 180 + 102);
        SuperPrint(list_level_exit_names[EditorCursor.WorldLevel.LevelExit[3]+1], 3, (e_ScreenW - 240)/2 + 10 + 44, ScreenH - 180 + 120);

        if (UpdateButton(10 + 4, ScreenH - 180 + 140 + 4, GFX.ECursor[1], false, 0, 0, 32, 32))
        {
            m_special_page = SPECIAL_PAGE_LEVEL_EXIT;
            m_special_subpage = 2;
        }
        SuperPrint("LEFT", 3, 10 + 44, ScreenH - 180 + 142);
        SuperPrint(list_level_exit_names[EditorCursor.WorldLevel.LevelExit[2]+1], 3, 10 + 44, ScreenH - 180 + 160);

        if (UpdateButton((e_ScreenW - 240)/2 + 10 + 4, ScreenH - 180 + 140 + 4, GFX.ECursor[1], false, 0, 0, 32, 32))
        {
            m_special_page = SPECIAL_PAGE_LEVEL_EXIT;
            m_special_subpage = 4;
        }
        SuperPrint("RIGHT", 3, (e_ScreenW - 240)/2 + 10 + 44, ScreenH - 180 + 142);
        SuperPrint(list_level_exit_names[EditorCursor.WorldLevel.LevelExit[4]+1], 3, (e_ScreenW - 240)/2 + 10 + 44, ScreenH - 180 + 160);
    }
...
void EditorScreen::UpdateSelectorBar()
{
    frmMain.renderRect(0, 0, e_ScreenW, 40, 0.8f, 0.8f, 0.8f, 1.0f, true);
    ...
    if (UpdateButton(0*40+4, 4, GFX.ECursor[2], !in_excl_special && EditorCursor.Mode == OptCursor_t::LVL_SELECT, 0, 0, 32, 32))
    {
        EditorCursor.Mode = OptCursor_t::LVL_SELECT;
        if (in_excl_special)
            exit_special = true;
    }
    if (UpdateButton(1*40+4, 4, GFX.ECursor[3], !in_excl_special && EditorCursor.Mode == OptCursor_t::LVL_ERASER0, 0, 0, 32, 32))
    {
    ...
    if (!WorldEditor)
    {
        if (UpdateButton(3*40+4, 4, GFXBlock[1], !in_excl_special && EditorCursor.Mode == OptCursor_t::LVL_BLOCKS, 0, 0, 32, 32))
        {
            EditorCursor.Mode = OptCursor_t::LVL_BLOCKS;
            if (in_excl_special)
                exit_special = true;
        }
        if (UpdateButton(4*40+4, 4, GFXBackgroundBMP[1], !in_excl_special && EditorCursor.Mode == OptCursor_t::LVL_BGOS, 0, 0, 32, 32))
        {
            EditorCursor.Mode = OptCursor_t::LVL_BGOS;
            if (in_excl_special)
                exit_special = true;
        }
        if (UpdateButton(5*40+4, 4, GFXNPC[1], !in_excl_special && EditorCursor.Mode == OptCursor_t::LVL_NPCS, 0, 0, 32, 32))
        {
            EditorCursor.Mode = OptCursor_t::LVL_NPCS;
            if (in_excl_special)
                exit_special = true;
        }
        if (UpdateButton(6*40+4, 4, GFXBackgroundBMP[26], !in_excl_special && EditorCursor.Mode == OptCursor_t::LVL_WATER, 0, 0, 32, 32))
        {
            EditorCursor.Mode = OptCursor_t::LVL_WATER;
            if (in_excl_special)
                exit_special = true;
        }
        if (UpdateButton(7*40+4, 4, GFXBlock[294], !in_excl_special && EditorCursor.Mode == OptCursor_t::LVL_WARPS, 0, 0, 32, 32))
        {
            EditorCursor.Mode = OptCursor_t::LVL_WARPS;
            if (in_excl_special)
                exit_special = true;
        }
        if (UpdateButton(9*40+4, 4, GFXBlock[60], !in_excl_special && EditorCursor.Mode == OptCursor_t::LVL_SETTINGS, 0, 0, 32, 32))
        {
            EditorCursor.Mode = OptCursor_t::LVL_SETTINGS;
            if (in_excl_special)
                exit_special = true;
        }
        if (UpdateButton(10*40+4, 4, GFXBlock[447], in_layers, 0, 0, 32, 32) && !in_layers)
        {
            EditorCursor.Mode = OptCursor_t::LVL_SELECT;
        ...
        if (UpdateButton(11*40+4, 4, GFXBlock[169], in_events, 0, 0, 32, 32) && !in_events)
        {
            EditorCursor.Mode = OptCursor_t::LVL_SELECT;
            optCursor.current = OptCursor_t::LVL_SELECT;
            m_last_mode = OptCursor_t::LVL_SELECT;
    ...
    if (WorldEditor)
    {
        if (UpdateButton(3*40+4, 4, GFXTileBMP[1], !in_excl_special && EditorCursor.Mode == OptCursor_t::WLD_TILES, 0, 0, 32, 32))
        {
            EditorCursor.Mode = OptCursor_t::WLD_TILES;
            if (in_excl_special)
                exit_special = true;
        }
        if (UpdateButton(4*40+4, 4, GFXSceneBMP[1], !in_excl_special && EditorCursor.Mode == OptCursor_t::WLD_SCENES, 0, 0, 32, 32))
        {
            EditorCursor.Mode = OptCursor_t::WLD_SCENES;
            if (in_excl_special)
                exit_special = true;
        }
        if (UpdateButton(5*40+4, 4, GFXLevelBMP[2], !in_excl_special && EditorCursor.Mode == OptCursor_t::WLD_LEVELS, 0, 0, 32, 32))
        {
            EditorCursor.Mode = OptCursor_t::WLD_LEVELS;
            if (in_excl_special)
                exit_special = true;
        }
        if (UpdateButton(6*40+4, 4, GFXPathBMP[4], !in_excl_special && EditorCursor.Mode == OptCursor_t::WLD_PATHS, 0, 0, 32, 32))
        {
            EditorCursor.Mode = OptCursor_t::WLD_PATHS;
            if (in_excl_special)
                exit_special = true;
        }
        if (UpdateButton(7*40+4, 4, GFX.NPC_modes, !in_excl_special && EditorCursor.Mode == OptCursor_t::WLD_MUSIC, 0, 32*26, 32, 32))
        {
            EditorCursor.Mode = OptCursor_t::WLD_MUSIC;
            if (in_excl_special)
                exit_special = true;
        }
        if (UpdateButton(9*40+4, 4, GFXLevelBMP[15], in_world_settings, 0, 0, 32, 32) && !in_world_settings)
        {
            EditorCursor.Mode = OptCursor_t::LVL_SELECT;
            optCursor.current = OptCursor_t::LVL_SELECT;
            m_last_mode = OptCursor_t::LVL_SELECT;
    ...
    // if (UpdateButton(14*40 + 4, 4, GFX.NPC_modes, false, 0, 32*22, 32, 32))
    // {
    //     if (WorldEditor)
    //         SaveWorld(FullFileName);
    //     else
    ...
    if (UpdateButton(15*40 + 4, 4, GFX.NPC_modes, false, 0, 32*31, 32, 32))
    {
...
{
    frmMain.initDraw(1);
    frmMain.renderRect(0, 0, e_ScreenW, ScreenH, 0.4f, 0.4f, 0.8f, 1.0f, true);
    UpdateSelectorBar();
