...
void SaveLevel(std::string FilePath)   // saves the level
{
    ...
    for(A = 0; A <= 100; A++)
    {
        ...
        for(B = 0; B <= 20; B++)
        {
            fwritenum(f, Events[A].Music[B]);
            fwritenum(f, Events[A].Background[B]);
            fwritenum(f, Events[A].level[B].X);
            fwritenum(f, Events[A].level[B].Y);
            fwritenum(f, Events[A].level[B].Height);
            fwritenum(f, Events[A].level[B].Width);
        }
